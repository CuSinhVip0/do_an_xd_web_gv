...
import { ExportExcel, ValidNumber } from "@/Utils";
export const Table = (
    props,
    { hasEdit = false, hasCheck = false, hasSave = false, userId = "", excel = "", nameExcel = "", setClear = () => {} }
) => {
    ...
    const handleExportExcel = () => {
        ExportExcel(edata, props.nameExcel);
    };
    const edata = data.map((item) => {
        return {
            "Mã môn học": item.mamonhoc,
            "Tên môn học": item.tenmonhoc,
            "Mã sinh viên": item.masinhvien,
            "Tên sinh viên": item.tensinhvien,
            "Học kì": item.hocki,
            "Năm học": item.nam,
            "Phần trăm quá trình": item.ptramquatrinh,
            "Phần trăm giữa kì": item.ptramgiuaki,
            "Phần trăm cuối kì": item.ptramcuoiki,
            "Điểm quá trình": item.diemquatrinh,
            "Điểm giữa kì": item.diemgiuaki,
            "Điểm cuối kì": item.diemcuoiki,
            "Xác nhận của phòng đào tạo": item.xacnhan,
        };
    });
    const handleSave = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${HOST}/api/score/add`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify(
                    data.map((item) => {
                        return {
                            ...
                            Year: item.nam,
                            Semester: `${item.hocki}`,
                            ProcessPercent: item.ptramquatrinh,
                            MidtermPercent: item.ptramgiuaki,
                            FinalPercent: item.ptramcuoiki,
                            ProcessScore: parseInt(item.diemquatrinh),
                            MidtermScore: parseInt(item.diemgiuaki),
                            FinalScore: parseInt(item.diemcuoiki),
                        };
                    })
                ),
            }).then((res) => res.json());
    ...
    const handleUpdate = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${HOST}/api/score/update`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${session.user.name}`,
                ...
                body: JSON.stringify({
                    StudentCode: dataUpdate.masinhvien,
                    ...
                    ProcessScore: parseInt(dataUpdate.diemquatrinh),
                    MidtermScore: parseInt(dataUpdate.diemgiuaki),
                    FinalScore: parseInt(dataUpdate.diemcuoiki),
                }),
            }).then((res) => res.json());
            if (res.message == "Cập nhật thành công.") {
                ...
                setData(
                    data.map((item) => {
                        if (
                            `${item.mamonhoc}-${item.masinhvien}-${item.hocki}-${item.nam}` ===
                            `${dataUpdate.mamonhoc}-${dataUpdate.masinhvien}-${dataUpdate.hocki}-${dataUpdate.nam}`
                        ) {
                            return {
                                ...item,
                                            ...
                                            <input
                                                ...
                                                checked={
                                                    listCheck.filter(
                                                        (item) =>
                                                            item ==
                                                            `${value.mamonhoc}-${value.masinhvien}-${value.hocki}-${value.nam}`
                                                    ).length > 0
                                                        ? true
                                                        : false
                                                }
                                                value={`${value.mamonhoc}-${value.masinhvien}-${value.hocki}-${value.nam}`}
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                                ...
                                {Object.keys(props.titledata).map((item, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                key={i}
                                                id={`${value.mamonhoc}-${value.masinhvien}-${value.hocki}-${value.nam}-${x[0]}`}
                                                className="block "
                                                contentEditable={
                                                    edit &&
                                                    (item == "diemquatrinh" ||
                                            ...
                                            >
                                                {x[0] == "xacnhan" ? (x[1] ? "x" : "") : x[1]}
                                            </div>
                                        </div>
                                    );
                                })}

                                {props.hasEdit && value.xacnhan == false && (
                                    <div className="table-cell py-4 w-full   border  text-center font-bold ">
                                        <button
                                            onClick={() => {
                                                if (dataOld != undefined || dataOld != null) {
                                                    document.getElementById(
                                                        `${dataOld.mamonhoc}-${dataOld.masinhvien}-${dataOld.hocki}-${dataOld.nam}-diemquatrinh`
                                                    ).innerText = dataOld.diemquatrinh;
                                                    document.getElementById(
                                                        `${dataOld.mamonhoc}-${dataOld.masinhvien}-${dataOld.hocki}-${dataOld.nam}-diemgiuaki`
                                                    ).innerText = dataOld.diemgiuaki;
                                                    document.getElementById(
                                                        `${dataOld.mamonhoc}-${dataOld.masinhvien}-${dataOld.hocki}-${dataOld.nam}-diemcuoiki`
